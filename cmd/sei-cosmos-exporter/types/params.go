...
// Default parameter values
var (
	DefaultVoteThreshold = cosmos_math.LegacyNewDecWithPrec(667, 3) // 66.7%
	DefaultRewardBand    = cosmos_math.LegacyNewDecWithPrec(2, 2)   // 2% (-1, 1)
	...
	DefaultSlashFraction     = cosmos_math.LegacyNewDecWithPrec(0, 4) // 0.00%
	DefaultMinValidPerWindow = cosmos_math.LegacyNewDecWithPrec(5, 2) // 5%
...
func (p Params) Validate() error {
	...
	if p.VoteThreshold.LTE(cosmos_math.LegacyNewDecWithPrec(33, 2)) {
		return fmt.Errorf("oracle parameter VoteThreshold must be greater than 33 percent")
	}

	if p.RewardBand.GT(cosmos_math.LegacyOneDec()) || p.RewardBand.IsNegative() {
		return fmt.Errorf("oracle parameter RewardBand must be between [0, 1]")
	}

	if p.SlashFraction.GT(cosmos_math.LegacyOneDec()) || p.SlashFraction.IsNegative() {
	...
	if p.MinValidPerWindow.GT(cosmos_math.LegacyOneDec()) || p.MinValidPerWindow.IsNegative() {
...
func validateVoteThreshold(i interface{}) error {
	...
	if v.GT(cosmos_math.LegacyOneDec()) {
